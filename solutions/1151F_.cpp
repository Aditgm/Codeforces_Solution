...
void solve(){
    ...
    ll invt=binpow(tot,mod-2,mod);
    L(x,0,c){
        ll inc=0,dec=0,same=0;
        if(c-x>=0 and c-x<=n-c){
            if(x+1<=c){
                inc=1ll*(c-x)*(c-x)%mod;
                matrix.m[x+1][x]=(inc*invt)%mod;
            }
            if(x-1>=0){
                dec=1ll*x*(n-2*c+x)%mod;
                if(dec>0)
                matrix.m[x-1][x]=(dec*invt)%mod;
            }
        }
        same=(tot-inc-dec)%mod;
        if(same<0) same+=mod;
        matrix.m[x][x]=(same*invt)%mod;
    }
    mat res=powm(matrix,k);
    cout<<(res.m[c][x]+mod)%mod<<endl;
}
 
signed main(){
