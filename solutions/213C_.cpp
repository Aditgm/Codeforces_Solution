...
void solve(){
    ...
    L(st,1,2*n-2){
        ...
        int prv=1-cur;
        L(i,0,n-1){
            L(j,0,n-1) dp[cur][i][j]=-inf;
        }
        int minr=max(0ll,st-(n-1));
        int maxr=min(n-1,st);
        L(i,minr,maxr){
            L(j,minr,maxr){
                int c1=st-i;
                int c2=st-j;
                int num=(i==j)?a[j][c2]:a[i][c1]+a[j][c2];
                int bst=-inf;
                if(c1-1>=0 and c2-1>=0) bst=max(bst,dp[prv][i][j]);
                if(i-1>=0 and c2-1>=0) bst=max(bst,dp[prv][i-1][j]);
                if(c1>=1 and j>=1) bst=max(bst,dp[prv][i][j-1]);
                if(i>=1 and j>=1) bst=max(bst,dp[prv][i-1][j-1]);
                if(bst!=-inf){
                    dp[cur][i][j]=bst+num;
                }
            }
