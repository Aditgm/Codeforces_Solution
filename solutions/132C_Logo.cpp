...
int vis[101][51][205][2];
int n;
string s;
void rec(int idx,int cnt,int pos,int dir){
    if(cnt>n or pos<0 or pos>200) return;
    if(vis[idx][cnt][pos][dir]) return;
    vis[idx][cnt][pos][dir]=1;
    if(idx==s.length()) return;
    int dirv=(dir==1)?1:-1;
    char ch=s[idx];
    if(ch=='F'){
        rec(idx+1,cnt,pos+dirv,dir);
    }
    else{
        rec(idx+1,cnt,pos,1-dir);
    }
    if(ch=='F'){
        rec(idx+1,cnt+1,pos,1-dir);
    }
    else{
        rec(idx+1,cnt+1,pos+dirv,dir);
    }
}
void solve(){
    cin>>s>>n;
