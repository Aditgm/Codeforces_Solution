...
ll g(ll x){
    if(x==0) return 0;
    return x+__builtin_popcountll(x)+((63-__builtin_clzll(x)));
}
ll get(ll x,ll v){
    ll st=((x+(1ll<<v)-1)>>v)<<v;
    ll bst=2e18;
    for(ll y=st;y<=st+30;y+=(1ll<<v)){
        bst=min(bst,g(y));
    }
    return bst;
}
void solve(){
    ...
    auto isv=[&](ll h,ll w)->bool{
        ...
        ll curh=h,curw=w;
        L(i,0,n-1){
            if(!bh.empty() and bh.rbegin()->first==curh){
                auto it=prev(bh.end());
                ll w=it->second;
                bw.erase(bw.find({w,curh}));
                bh.erase(it);
                curw-=w;
            }
            else if(!bw.empty() and bw.rbegin()->first==curw){
                auto it=prev(bw.end());
                ll h=it->second;
                bh.erase(bh.find({h,curw}));
                bw.erase(it);
                curh-=h;
            }
            else return 0;
    ...
    vector<pll> ans;
    if(tot%maxh==0 and isv(maxh,tot/maxh)){
        ans.pb({maxh,tot/maxh});
    }
    ll h2=tot/maxw;
    if(tot%maxw==0 and isv(h2,maxw)){
        if(ans.empty() or ans[0]!=make_pair(h2,maxw)){
            ans.pb({h2,maxw});
        }
    }
    cout<<ans.size()<<endl;
