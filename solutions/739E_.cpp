...
void solve(){
    ...
    L(st,1,50){
        ...
        double lowu=0.0,highu=1.0;;
        int cur=0;
        L(st1,1,50){
            double midu=lowu+(highu-lowu)/2.0;
            node1 res={0.0,0,0};
            L(i,0,n-1){
                double none=0.0;
                double poke=p[i]-mid;
                double ultra=u[i]-midu;
                double both=(p[i]+u[i]-p[i]*u[i])-mid-midu;
                double maxi1=max({none,poke,ultra,both});
                res.expi+=maxi1;
                if(abs(maxi1-both)<eps){
                    res.useda++;
                    res.usedb++;
                }
                else if(abs(maxi1-poke)<eps){
                    res.useda++;
                }
                else if(abs(maxi1-ultra)<eps) res.usedb++;
            }
            if(res.usedb>b){
                lowu=midu;
            }
    ...
    cout<<fixed<<setprecision(6)<<bst<<endl;
}
 
signed main(){
