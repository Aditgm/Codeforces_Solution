...
void dfs(int u,int p){
    ...
    for(auto v:tree[u]){
        if(v!=p and v!=heavy){
            int off=vec[u]->size()-vec[v]->size();
            L(i,0,vec[v]->size()-1){
                int trget=off+i;
                (*vec[u])[trget]+=(*vec[v])[i];
                if((*vec[u])[trget]>(*vec[u])[bst[u]]){
                    bst[u]=trget;
                }
                else if((*vec[u])[trget]==(*vec[u])[bst[u]] and trget>bst[u]){
                    bst[u]=trget;
                }
            }
        }
