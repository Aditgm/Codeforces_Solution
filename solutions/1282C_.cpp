...
void solve(){
  ...
  sort(all(p));
  ll maxi=0,req=0,reasy=0,rhard=0;
  L(i,0,n){
    ll cur=(i==n)?t:p[i].t-1;
    if(req<=cur){
        ll rem=cur-req;
        ll remeasy=easy-reasy;
        ll takeeasy=min(remeasy,rem/a);
        rem-=takeeasy*a;
        ll remhard=hard-rhard;
        ll takehard=min(remhard,rem/b);
        maxi=max(maxi,reasy+rhard+takeeasy+takehard);
    }
    if(i<n){
        if(p[i].type==0){
            reasy++;
